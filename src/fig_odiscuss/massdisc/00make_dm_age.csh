...
#  set y = `echo "scale = 10;(($fit_t - 30000) / 10000) +1" | bc`
  ...
  if ($?fill) then
    echo sci $fill >> $file
    echo dot $age[3] $dm 1 $type >> $file
    echo sci 1 >> $file
    echo sci $fill >> b_$file
    echo dot $age[5] $dm 1 $type >> b_$file
    echo sci 1 >> b_$file
  endif

  echo dot $age[3] $dm 2 $type >> $file
  echo sch 1 >> $file
  echo dot $age[5] $dm 2 $type >> b_$file
  echo sch 1 >> b_$file

  echo $age[3] $dm >> prob1.asc
  echo $age[5] $dm >> prob2.asc
end
...
dot 5.05 44200 2 20
sch 1.0
ptext 0.0 "\fiy\fr = 0.09" 5.2 44000 0.0

sch 1.2
dot 4.85 42200 2 4
dot 5.05 42200 2 20
sch 1.0
ptext 0.0 "\fiy\fr = 0.12" 5.2 42000 0.0

sch 1.5
dot 4.85 40200 2 4
dot 5.05 40200 2 20
sch 1.0
ptext 0.0 "\fiy\fr = 0.15" 5.2 40000 0.0
...
if (-e gks74.ps) then
  ...
  mv gks74.ps dm_age.eps
endif

rm *.cmd *.asc junk.$$ lincor*
