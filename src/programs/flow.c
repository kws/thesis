...
flow.c 22/1/93 Terry Brown  Last mod: 31/10/93

flow - a flow-chart -> LaTeX generator.  Parses a file in a flow-chart
specification language, then produes a corresponding LaTeX
pic environment.
...
*/
    ...
    strcpy(plist, line+strlen(fcom[command].name));
...
*/
    ...
    default:
        fprintf(stderr,"flow error : unknown command %3.4f\n",command);
        return 0;
    }
